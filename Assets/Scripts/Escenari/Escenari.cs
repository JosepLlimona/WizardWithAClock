...
static class Escenari
{
    static public int[,] SpawnEscenari(Vector2 size, int quadratsPerGenerar, float dpc, Vector2 midaParets)
    {
        ...
        {
            return SpawnEscenari(size, quadratsPerGenerar, dpc, midaParets);
        }
        
        return MapaEscenari;
    }

    static int[,] CrearQuadratsEscenari(int size_x, int size_y, int nQuadrats, float dpc)
    {
        ...
        for (int i = 0; i < nQuadrats; i++)
        {
            ...
            for (int j = salaPos_X; j < salaPos_X + midaSala_X; j++)
            {
                for (int k = salaPos_Y; k < salaPos_Y + midaSala_Y; k++)
                {
                    newMapa[j, k] = 1;
                }
            }
    ...
    static int TrobarSales(int[,] MapaEscenari, int size_x, int size_y)
    {
                ...
                if (mapaModificat[i, j] == 1)
                {
                    ...
                    while (possiblesCoord.Count > 0)
                    {
                        ...
                        for (int aux_x = x - 1; aux_x <= x + 1; aux_x++)
                        {
                            ...
                            {
                                if (aux_x >= 0 && aux_x < size_x && aux_y >= 0 && aux_y < size_y &&
                                    mapaModificat[aux_x, aux_y] == 1)
                                {
    ...
    static private int[,] CrearParetsEscenari(int[,] MapaEscenari, int size_x, int size_y, int nSales, Vector2 midaParets)
    {
        ...
        for (int passadisNum = 1; passadisNum <= nSales; passadisNum++)
        {
            ...
            while (MapaEscenari[x1, y1] != passadisNum && nSalesProb < nSalesProbMax)
            {
                x1 = Random.Range(1, size_x - 1);
                y1 = Random.Range(1, size_y - 1);
                nSalesProb++;
            ...
            int xDirection = dife_x != 0 ? dife_x / Mathf.Abs(dife_x) : 0;
            int yDirection = dife_y != 0 ? dife_y / Mathf.Abs(dife_y) : 0;
